...
def test_create_creates_project_with_name():
    with ThetaProject.create(name="my-agent") as proj:
        assert proj.name == "my-agent"
        assert proj._manifest_path.exists()


def test_create_cleans_up_on_exit():
    with ThetaProject.create(name="cleanup-test") as proj:
        temp_dir = proj._temp_dir
    assert not temp_dir.exists()


def test_create_sync_materializes_system_and_rules():
    with ThetaProject.create(name="sync-test") as proj:
...
def test_create_identity_before_sync():
    with ThetaProject.create(name="identity-agent") as proj:
...
def test_create_rules_empty_when_only_system_synced():
    with ThetaProject.create(name="no-rules") as proj:
        proj.add.system(content="just a system prompt")
        proj.sync(validate=False)
        assert proj.rules == {}


def test_create_lock_hash_after_sync():
    with ThetaProject.create(name="hash-test") as proj:
...
def test_from_manifest_does_not_touch_source(tmp_path):
    ...
    with ThetaProject.from_manifest(tmp_path / "theta.toml"):
        pass
...
def test_from_manifest_no_sync_requires_manual_sync(tmp_path):
    from theta_py import runner

    runner.run(["init", "--name", "existing-agent"], cwd=tmp_path)
    runner.run(["add", "system", "--content", "Existing system."], cwd=tmp_path)

    with ThetaProject.from_manifest(tmp_path / "theta.toml", no_sync=True) as proj:
...
def test_system_prompt_raises_before_sync():
    with ThetaProject.create(name="unsync-guard") as proj, pytest.raises(RuntimeError, match="sync"):
        _ = proj.system_prompt


def test_rules_raises_before_sync():
    with ThetaProject.create(name="rules-guard") as proj, pytest.raises(RuntimeError, match="sync"):
        _ = proj.rules


def test_skills_path_raises_before_sync():
    with ThetaProject.create(name="skills-guard") as proj, pytest.raises(RuntimeError, match="sync"):
        _ = proj.skills_path


def test_tools_empty_on_fresh_project():
    with ThetaProject.create(name="no-tools") as proj:
        proj.add.system(content="placeholder")
        proj.sync(validate=False)
        assert proj.tools == {}


def test_repr_reports_synced_after_sync_even_without_theta_dir():
    with ThetaProject.create(name="repr-sync") as proj:
        proj.sync(validate=False)
        assert "synced" in repr(proj)
